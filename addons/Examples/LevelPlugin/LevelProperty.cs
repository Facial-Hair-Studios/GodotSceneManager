...
public partial class LevelProperty : EditorProperty
{
        ...
        if (Load())
        {
            ...
            if (_managerData.Levels != null)
            {
                foreach (var entry in _managerData.Levels)
                {
                    ...
                    if (level is LevelSelect && level is not Level) // We only care about Level Selects!
                    {
    ...
    public override void _Ready()
    {
        ...
        _currentlySelectedGuid = (String)GetEditedObject().Get("ParentLevel");
